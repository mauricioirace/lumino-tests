...
export function waitForHealthCheck({command, container, expectedResult, timeoutMS = 120000, waitBeforeStart = 1000}: {
    ...
    const healthCheck = async () => {
        ...
        console.debug(`HealthCheck Command Result for container ${container.getName()}`, result);
    ...
    return new Promise(async (resolve, reject) => {
        const waitBeforeStartTimeout = setTimeout(async () => {
            const healthCheckTimeout = setTimeout(async () => {
                reject(`HealthCheck timeout for container ${container.getName()}`);
                clearTimeout(healthCheckTimeout);
            }, timeoutMS);
            while(!await healthCheck()) {
                console.debug(`Retrying healthCheck for container ${container.getName()}`);
            }
            resolve();
