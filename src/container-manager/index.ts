...
export default class ContainerManager {
    ...
    public async startRskNode(): Promise<StartedDockerComposeEnvironment> {
        ...
        this.rskNodeEnvironment = await this.startDockerComposeModule(path.resolve(__dirname, 'docker-files/rsk-node'));
    ...
    public async startExplorer(): Promise<StartedDockerComposeEnvironment> {
        ...
        this.explorerEnvironment = await this.startDockerComposeModule(path.resolve(__dirname, 'docker-files/lumino-explorer'));
        await waitForHealthCheck({
            command: 'healthCheck',
            expectedResult: 'NODE ONLINE',
    ...
    private async startDockerComposeModule(buildContext: string): Promise<StartedDockerComposeEnvironment> {
    ...
    public async startupLuminoNode(nodeConfig: SetupNode): Promise<LuminoNode> {
        ...
        luminoNodeContainer = luminoNodeContainer.withNetworkMode(DOCKER_NETWORK_NAME);

        if (nodeConfig.enableHub) {
            console.debug(`Setup Hub Mode to lumino node ${nodeConfig.name}`);
            luminoNodeContainer = luminoNodeContainer.withEnv('HUB_MODE', 'enabled');
        }
            ...
            for (const tokenSetup of nodeConfig.tokens) {
                switch (tokenSetup.symbol) {
                    case Token.LUM.toString():
                        console.debug(`Setup ${tokenSetup.amount} lumino tokens to lumino node ${nodeConfig.name}`);
                        luminoNodeContainer = luminoNodeContainer.withEnv('LUM_FUNDS', tokenSetup.amount.toString());
                        break;
                    case Token.RIF.toString():
                        console.debug(`Setup ${tokenSetup.amount} rif tokens to lumino node ${nodeConfig.name}`);
                        luminoNodeContainer = luminoNodeContainer.withEnv('RIF_FUNDS', tokenSetup.amount.toString());
                        break;
                }
            }
        }
        ...
        const containerIp: string = startedContainer.getIpAddress(DOCKER_NETWORK_NAME);

        const luminoNodeBaseUrl = `http://${containerIp}:5001/api/v1`;

        console.debug(`Creating sdk for lumino node ${nodeConfig.name} with luminoNodeBaseUrl=${luminoNodeBaseUrl}`);
