...
export default class ChannelManager {
    ...
    async openChannels(channels: SetupChannel[], nodes: Node[]): Promise<void> {
        await Promise.all(channels.map((channel) => this.openChannel(channel, nodes)));
    }
    async openChannel({ tokenSymbol, participant1, participant2 }: SetupChannel, nodes: Node[]): Promise<void> {
