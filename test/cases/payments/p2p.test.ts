    ...
    test(
        ...
        async () => {
            ...
            await given(nodes.target).expectChannel({
                tokenAddress: tokenAddresses.LUM,
                partnerAddress: nodes.initiator.client.address
            }).toHaveBalance(targetDeposit + initiatorPaymentAmount - targetPaymentAmount);
            // repeat verification from "initiator" node
            await given(nodes.initiator).expectChannel({
                tokenAddress: tokenAddresses.LUM,
                partnerAddress: nodes.target.client.address
            }).toHaveBalance(initiatorDeposit - initiatorPaymentAmount + targetPaymentAmount);
        },
        TEST_TIMEOUT
    );
});
