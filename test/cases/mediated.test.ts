...
describe("Mediated Payments", () => {
  ...
  it(
    ...
    async () => {
      ...
      expect(initiatorMediatorChannel.balance).toBe(
        oldInitiatorMediatorChannel.balance - toWei(1)
      );
      expect(mediatoInitiatorrChannel.balance).toBe(
        oldMediatoInitiatorrChannel.balance + toWei(1)
      );
      expect(mediatorTargetChannel.balance).toBe(
        oldMediatorTargetChannel.balance - toWei(1)
      );
      expect(targetMediatorChannel.balance).toBe(
        oldTargetMediatorChannel.balance + toWei(1)
      );
    },
    TIMEOUT
  );
